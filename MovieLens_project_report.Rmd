...
2.  **Magnitude over Binary Logic**: Unlike binary accuracy, RMSE accounts for the magnitude of the error.
3.  **Statistical Robustness**: It provides a smooth, differentiable surface that is ideal for the optimization (Fine-tuning) of our regularization parameters ($\lambda$).
...
In this equation: - $\mu$ represents the "true" average rating across all movies and users. - $\epsilon_{u,i}$ represents the independent errors centered at zero.
...
To ensure the model's robustness and handle the noise inherent in small sample sizes (such as movies with very few ratings or inactive users), we implemented **Regularization**. Furthermore, we refined our bias estimates using a **Backfitting** approach and ensured the realism of our predictions through **Capping** logic.

## Final Results and Achievements

Our modeling process followed a rigorous methodology, using a 10% test split for cross-validation and a final hold-out set for ultimate verification. The final results are summarized below:
...
The final RMSE of **`r round(final_holdout_rmse, 5)`** successfully met the project target of **0.86490**. This confirms that the identified biases (especially the movie and user effects) carry significant predictive power.

## Limitations and Future Work

While the current model performs well, several areas for improvement remain:

1.  **Matrix Factorization**: Although our bias-based model is efficient, advanced techniques like SVD (Singular Value Decomposition) or ALS (Alternating Least Squares) could capture deeper latent patterns in user-movie interactions that simple additive biases cannot.
2.  **Computational Constraints**: The analysis was strictly performed under a 6 GB RAM constraint. With more computational resources, we could implement more complex ensemble methods or deep learning architectures (like Neural Collaborative Filtering).
3.  **Cold Start Problem**: The model still struggles with new users or movies with no prior history. Integrating metadata (like movie plot summaries or user demographics) could help mitigate this.

As discussed in the evaluation section, while our model achieved the target RMSE, future iterations could focus on optimizing for ranking metrics like nDCG to better reflect real-world user satisfaction beyond point-prediction accuracy.

In conclusion, this project demonstrates that a carefully tuned linear model with regularization and feature engineering can achieve high precision, providing a solid foundation for more advanced recommendation engines.
